...
//Create a function that is able to pull the login info
function getLogin($userName, $password)
{
   ...
   //Create an SQL Query
   $stmt = $db ->prepare("SELECT * FROM users WHERE userName = :userNameParam AND userPassword = :passwordParam");

   //Bind the values
    $stmt->bindValue(':userNameParam', $userName);
    $stmt->bindValue(':passwordParam', $password);

    if ($stmt-> execute() && $stmt->rowCount() > 0)
    {
        $results = "Login was Successful";
    }
