...
//  5) Return results if needed.
    ...
    public function getTeams() 
    {
        ...
        // Preparing SQL query
        $stmt = $teamTable->prepare("SELECT * FROM book ORDER BY title"); 
    ...
    public function addTeam($title, $author, $releaseDate, $isbn, $genre) 
    {
        $addSucessful = false;         // Team not added at this point
        $teamTable = $this->teamData;   // Alias for database PDO

        // Preparing SQL query with parameters for team and division
        $stmt = $teamTable->prepare("INSERT INTO book SET title = :titleParam, author = :authorParam, releaseDate = :releaseDateParam, isbn = :isbnParam, genre = :genreParam");
    ...
    // INPUT: team and divison to add
    // RETURNS: True if add is successful, false otherwise
    public function addTeam2($title, $author, $releaseDate, $isbn, $genre) 
    {
        $addSucessful = false;         // Team not added at this point
        $teamTable = $this->teamData;   // Alias for database PDO
    ...
    public function updateTeam ($id, $title, $author, $releaseDate, $isbn, $genre) 
    {
        ...
        $stmt = $teamTable->prepare("UPDATE book SET title = :titleParam, author = :authorParam, releaseDate = :releaseDateParam, isbn = :isbnParam, genre = :genreParam WHERE id=:idParam");
    ...
    public function deleteTeam ($id) 
    {
        ...
        $stmt = $teamTable->prepare("DELETE FROM book WHERE id=:idParam");
    ...
    public function getTeam ($id) 
    {
        ...
        // Preparing SQL query 
        //    id is used to ensure we delete correct record
        $stmt = $teamTable->prepare("SELECT id, title, author, releaseDate, isbn, genre FROM book WHERE id=:idParam");
